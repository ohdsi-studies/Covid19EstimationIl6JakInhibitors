...
getMainResults <- function(exposureIds = c(),
                           outcomeIds = c(),
                           databaseIds = c(),
                           analysisIds = c()) {
  idx <- rep(TRUE, nrow(sccsResult))
  if (length(exposureIds) != 0) {
     idx <- idx & sccsResult$exposureId %in% exposureIds
  }
  if (length(outcomeIds) != 0) {
    idx <- idx & sccsResult$outcomeId %in% outcomeIds
  }
  if (length(databaseIds) != 0) {
    idx <- idx & sccsResult$databaseId %in% databaseIds
  }
  if (length(analysisIds) != 0) {
    idx <- idx & sccsResult$analysisId %in% analysisIds
  }
  return(sccsResult[idx, ])
}
